...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Fetch up to 6 related articles (same category, same location if possible)
  const [sameCat, sameLocation] = await Promise.all([
    getPublishedArticles(8, article.category),
    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-10 xl:gap-14">
          ...
          {/* ── Article ── */}
          <article>
            <Link href="/news" className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-400 hover:text-zinc-900 transition-colors mb-6 uppercase tracking-wider">
              <ArrowLeft className="h-3.5 w-3.5" /> News
            </Link>

            <div className="flex items-center gap-2 mb-3">
              <span className="text-[10px] font-black text-[#00cc6a] uppercase tracking-wider">News</span>
              {article.location && (
                <>
