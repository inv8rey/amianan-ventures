...
import { Loader2, CheckCircle, MessageSquare, XCircle, Globe } from 'lucide-react'
...
type ActionMode = null | 'revision' | 'reject' | 'publish'
...
export function EditorActions({ submission }: EditorActionsProps) {
  ...
  return (
    <div className="rounded-lg border border-border/40 bg-card p-4 space-y-4">
        ...
        <div className="space-y-2">
          ...
          {mode !== 'reject' ? (
            ...
            <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3 space-y-2">
              ...
              <p className="text-[10px] text-muted-foreground">A rejection notice will be sent to the contributor. You can add a note to your private editor notes above.</p>
              <div className="flex gap-2">
                <button
      ...
      {/* ── Mark as Published (only when approved) ─────────────── */}
      {isApproved && (
        <div>
          {mode !== 'publish' ? (
            <button
            ...
            <div className="rounded-md border border-[#00a855]/30 bg-[#00a855]/5 p-3 space-y-2">
              <label className="block text-xs font-semibold text-[#00a855]">Published URL</label>
              <input
                type="url"
                value={publishedUrl}
                ...
                autoFocus
                className="w-full rounded-md border border-[#00a855]/30 bg-background px-3 py-2 text-xs text-foreground placeholder-muted-foreground outline-none focus:border-[#00a855]/60"
              />
              <div className="flex gap-2">
                <button
                  onClick={() => updateStatus('published', { publishedUrl })}
                  disabled={saving || !publishedUrl.trim()}
